...
docker tag $MODEL_NAME:latest $AWS_REGISTRY_URL/$MODEL_NAME:$DOCKER_IMAGE_TAG
...
set +e
kubectl create namespace $MODEL_NAME
set -e

# Install helm chart to the EKS cluster via helm
helm install $MODEL_NAME ./helm_chart \
...
# port forward
kubectl port-forward service/$MODEL_NAME -n $MODEL_NAME $DEST_PORT:80
